...
export class WechatClient {
	private static instance: WechatClient | null = null;
	...
	public static getInstance(plugin: One2MpPlugin): WechatClient {
	...
	public static resetInstance() {
		WechatClient.instance = null;
	}

	public async requestToken(): Promise<string | null> {
		// 中心服务器模式：由服务端代取 access_token
	...
	public async getAccountBasicInfo(accountName?: string) {
		const targetAccount =
		...
		const accessToken = await this.plugin.refreshAccessToken(targetAccount);
		if (!accessToken) {
			return null;
		}
		const url = `${this.baseUrl}/account/getaccountbasicinfo?access_token=${accessToken}`;
		const res = await requestUrl({
			method: "GET",
			url,
			throw: false,
		});
		const { errcode } = res.json;
			...
			articles: [
				{
					...
					...(localDraft.pic_crop_235_1 && {
						pic_crop_235_1: localDraft.pic_crop_235_1,
					}),
					...(localDraft.pic_crop_1_1 && {
						pic_crop_1_1: localDraft.pic_crop_1_1,
					}),
	...
	async uploadMaterial(data: Blob, filename: string, type?: string) {
		...
		try {
			...
			if (resData.errcode !== undefined && resData.errcode !== 0) {
				console.error("上传素材失败:", resData);
				new Notice(
					getErrorMessage(resData.errcode) || resData.errmsg || "上传素材失败",
					0
				);
				return null;
			}
	...
	public async publishDraft(meida_id: string, accountName: string = "") {
		...
		const { errcode, publish_id } = resp.json;
		if (errcode !== undefined && errcode !== 0) {
			new Notice(getErrorMessage(errcode), 0);
			return false;
		} else {
